...
block1(X,Y) :- nearEnemyMoveCell(X,Y,_,_,_,3). % cells from where myUnit can block enemyUnit to win
...
% 5)prefer moving on higher cell 
:~ moveIn(X,Y,H), myUnit(_,_,Hmy,_), H<=Hmy, &sum(H,-Hmy;Z), &abs(Z;W). [W+1@4] 
%%%AGGRESSIVE STRATEGY
:~ moveIn(X,Y,H), not enemyMoveCell(X,Y,H). [2@3] % penalty if don't move to an enemyMoveCell
:~ moveIn(X,Y,H), not nearEnemyMoveCell(X,Y,H,Xm,Ym,Hm), moveCell(Xm,Ym,Hm). [1@3] % penalty if don't move to a nearEnemyMoveCell
